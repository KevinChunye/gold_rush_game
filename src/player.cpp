// player.cpp — GoldRush 2.0 tournament bot.
//
// Deliberately simple, built around two invariants that decide matches
// before strategy does: it ALWAYS returns a legal GameOutput (a malformed
// answer or crash forfeits instantly), and it answers in ~1.5 us — the
// engine executes the faster player's moves first each round, so latency
// is itself a weapon: we take first pick of every contested pile.
//
// Strategy per round:
